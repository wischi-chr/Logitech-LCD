...
namespace LgLcd
{
    ...
    public abstract class Applet : IApplet
    {
        ...
        static Applet()
        {
            ...
            AppDomain.CurrentDomain.ProcessExit += delegate { LgLcd.DeInit(); };
        }
        ...
        public void Connect(string friendlyName, bool autostartable, AppletCapabilities appletCaps)
        {
            ...
            if (error != LgLcd.ReturnValue.ErrorSuccess)
            {
                switch (error)
                {
                    case LgLcd.ReturnValue.ErrorInvalidParameter: throw new ArgumentException("FriendlyName must not be null.");
                    case LgLcd.ReturnValue.ErrorFileNotFound: throw new Exception("LCDMon is not running on the system.");
                    case LgLcd.ReturnValue.ErrorAlreadyExists: throw new Exception("The same client is already connected.");
                    case LgLcd.ReturnValue.RcpXWrongPipeVersion: throw new Exception("LCDMon does not understand the protocol.");
                    default: throw new Win32Exception((int)error);
                }
            }
